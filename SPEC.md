...
- [x] Полный diff, явное подтверждение владельца перед commit/push
  (CONSTITUTION, без исключений). Коммит `e388bea` в `master`, запушен.
- [x] Реальный приёмочный прогон в CI (Правило 31) — выполнен 05.08.2026,
  pipeline `#2733648189` (schedule, `$LIFECYCLE_ONLY=true`), Passed. См.
  §13 ниже.
...
### 13. Реальный приёмочный прогон (Правило 31) — результат

Выполнено 05.08.2026, pipeline `#2733648189` (source: Scheduled,
`$LIFECYCLE_ONLY=true`, branch `master`, commit `e388beae`). **Passed**, 2
job'а в стадии `run`: `lint_vault` и `recheck_lifecycle` — `radar` и
`promote_candidates` в этом pipeline не появились вообще, подтверждает
таблицу правок `rules` из §12 на практике, не только по чтению YAML.

`recheck_lifecycle`: 72 `VALIDATED_SHIFT`-файла найдено, 69 успешно
обработаны и запушены в vault (`git commit e4efcad` в ветке `vault`,
`check_frontmatter.py`-гейт прошёл). Три файла не обработаны, оба класса
ожидаемы по дизайну §9/Graceful Degradation, не баги этой сессии:

- 1 репозиторий недоступен через GitHub API (404,
  `Cyborg7-com/cyborg`) — файл целиком пропущен в этом прогоне, не
  трактован как мёртвый (§9).
- 2 файла — ошибка `parse_github_owner_repo()` на данных `**Репозиторий:**`
  в теле файла: один указывает на не-GitHub URL
  (`https://hyperterse.com`), другой имеет посторонний текст, дописанный
  на той же строке после URL. Тот же `read_repo_url()`, что уже
  используется `promote_candidates.py` (§10, сознательно продублирован,
  не вынесен в общий модуль) — существующая характеристика данных vault,
  не новая проблема кода этой сессии. Оба случая залогированы с чёткой
  причиной, batch не прерван.

Большинство обработанных файлов получили `license_baseline_origin:
"migration"` (первый прогон для файлов без `license_spdx_id` — все 88
backfilled + все файлы, оценённые до этой сессии) — baseline проставлен,
`license_changed`-событие не написано (первое наблюдение, не изменение),
ровно по дизайну §4/§7.
...
8. [x] `.gitlab-ci.yml`: job `recheck_lifecycle` + правки `rules` в
   `radar`/`promote_candidates` (§12).
9. [x] Показать финальный список `rules:` для всех затронутых job'ов
   владельцу до коммита `.gitlab-ci.yml` — подтверждено владельцем.
10. [x] Полный diff, явное подтверждение владельца перед commit/push —
    коммит `e388bea`, запушен в `origin/master`.
11. [x] Новый GitLab Schedule (`$LIFECYCLE_ONLY=="true"`) создан
    владельцем, реальный приёмочный прогон в CI (Правило 31) выполнен —
    pipeline `#2733648189`, Passed. Результат см. §13.

Все Milestones закрыты. Задача завершена.
