...
| CI/CD (9 GitHub Actions workflow files + `vault-write` composite action) | Implemented — 2 of 9 workflows confirmed running by trigger; the other 7 are cron-scheduled and had not fired even once as of 2026-08-25 (see docs/BACKLOG.md [B-003]) | Collection & filtering, Assessment, Quarantine resolution, Lifecycle recheck, Pattern clustering & falsification, External analyst input, Model-update feedback check, Publishing, Vault write mechanism, Site generation | `test.yml` and `pages.yml` confirmed running (push-triggered). All 6 secrets (`ANTHROPIC_API_KEY`, `GH_READ_TOKEN`, `GH_VAULT_PUSH_TOKEN`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHANNEL_ID`, `TELEGRAM_OWNER_ID`) rebuilt fresh in GitHub Actions Secrets, none copied from the former GitLab CI/CD Variables. GitHub Actions runners use `python:3.12` via `actions/setup-python` | `fd6e8b4` |
