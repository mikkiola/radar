...
| GitHub Pages | Implemented | Site generation, CI/CD | One successful `pages.yml` run confirmed (2026-08-25T13:23:40Z, build and deploy both succeeded); site substantively works — home page and Patterns/Assessments sections render, all 14 migrated patterns display correctly. Interactive graph confirmed working: `docs/assets/javascripts/interactive_graph.js:2` fetches `/assets/javascripts/graph.json`, which returns 200 with valid JSON data (`curl`-confirmed 2026-09-18) — the originally-reported 404 was on a different, never-fetched path; see docs/BACKLOG.md [B-001] (closed, resolved as misdiagnosed) | `fd6e8b4` |
| Platform: GitHub repo hosting (`github.com/mikkiola/radar`) | Implemented | — | Full SHA parity confirmed between GitLab and GitHub for both `main` and `vault` before GitLab deletion; `vault` moved as a full branch with complete history (1879 objects), SHA `39286a667923f64418b08942df926f6ca23571c3` confirmed identical on both platforms pre-deletion. `main`'s branch ruleset (deletion/non-fast-forward/update, single bypass actor: RepositoryRole admin, "always") removed 2026-09-23 — sole collaborator already held unconditional admin bypass, so it protected nothing for the only actor with repo access; `main` now has no branch protection, direct push; `vault` also unprotected — see docs/BACKLOG.md [B-002] | `fd6e8b4` (`.gitlab-ci.yml` removal, workflow files added); branch rename/protection/ruleset changes are not commit-tracked |
